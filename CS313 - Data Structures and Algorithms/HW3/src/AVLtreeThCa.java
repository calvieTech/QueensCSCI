...
public class AVLtreeThCa<E extends Comparable<? super E>>{
    ...
    public int range(AVLtreeThCa<E> myBST) {
        if(myBST.getRoot() == null) return 0;
        AVLNode<E> min = findMin(myBST.getRoot());
        AVLNode<E> max = findMax(myBST.getRoot());
        int numberMin = (Integer) min.getE();
        int numberMax = (Integer) max.getE();
        int difference = numberMax - numberMin;
        return difference;
}

    public static void main(String argc[]){
        AVLtreeThCa<Integer> tr = new AVLtreeThCa<Integer>();
        System.out.println("Tree Range:" + tr.range(tr));
        tr.add(4);
        tr.add(5);
