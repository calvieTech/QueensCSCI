//Calvin Thang
//Lab 11A

import javax.swing.*;
import java.awt.*;
import java.util.ArrayList;

public class Date212 {
   private int year, month, day;
   public static ArrayList<Integer> arrayOfDates = new ArrayList<Integer>();
   public int size;
   
   public Date212() {
      throw new IllegalArgumentException("Must have a string parameter");
   }
   
   
   public Date212(String date) {
      
      //extract from the string date
      year = Integer.parseInt(date.substring(0, 4));
      month = Integer.parseInt(date.substring(4, 6));
      day = Integer.parseInt(date.substring(6, 8));
      
      //check if input is valid
      if (!checkValidation(month, day)) {
         System.out.println("Not a valid month or day");
      } else {
         //add it to the array
         arrayOfDates.add(Integer.parseInt(date));
      }
      
      
      
   }
   
   //get and set methods with validation for input
   public int getYear() {
      return year;
   }
   
   public int getMonth() {
      return month;
   }
   
   public int getDay() {
      return day;
   }
   
   public void setYear(int yearInput) {
      year = yearInput;
   }
   
   public void setMonth(int monthInput) {
      if (monthInput <= 0 || monthInput > 12) throw new IllegalArgumentException("invalid month");
      month = monthInput;
   }
   
   public void setDay(int dayInput) {
      if (dayInput <= 0 || dayInput > 31) throw new IllegalArgumentException("invalid day");
      day = dayInput;
   }
   
   
   private boolean checkValidation(int monthInput, int dayInput) {
      if (monthInput <= 0 || monthInput > 12 || dayInput <= 0 || dayInput > 31)
         return false;
      else
         return true;
   }
   
   
   //convert attributes to type String using get methods
   public String toString() {
      String dateYear = Integer.toString(this.getYear());
      String dateMonth = Integer.toString(this.getMonth());
      String dateDay = Integer.toString(this.getDay());
      return (dateMonth + "/" + dateDay + "/" + dateYear);
      
      
   }   

}